...
{
  /// <summary>Where the sounds are. See <see cref="GamePaths.ContentRoot"/>.</summary>
  public static string CONTENTFOLDER => GamePaths.ContentRoot;
  ...
  {
    Sound sound = AmbiSounds[GameHost.Current.Random.Next(AmbiSounds.Count())];
    Channel channel = System.PlaySound(sound, paused: false)!;
    channel.Set3DAttributes(new Vector3(GameHost.Current.Random.Next(-20, 20), GameHost.Current.Random.Next(-20, 20), 5f), default, default);
    channel.Set3DMinMaxDistance(0, 35);
    bool enabled = _ambiTimer.Enabled;
    _ambiTimer?.Dispose();
    _ambiTimer = new Timer(GameHost.Current.Random.Next(4000, 8000));
    _ambiTimer.Elapsed += onAmbiTimer;
  ...
  public void DropFruitSound(FruitSpecies fruitSpecies)
  {
  ...
  {
    //GameHost.Current.GameWindow?.DisableInput();
    ...
    //GameHost.Current.GameWindow?.EnableInput();
  }

  public void PlaySadMusic()
  {
    MusicTransition(GameHost.Current.SoundSystem.SadMusicStream, 464375, 4471817, TimeUnit.PCM, 0.1f);
  }
  public void PlayNWelcomeMusic()
  {
  ...
  public void MusicFadeOut(int seconds = 2)
  {
    if (Music != null)
    {
      do
  ...
  public void UpdateWaterPoints(Map map, Vector3 playerPosition)
  {
